...
class EventDispatcher:
    ...
    :param callback: Function to hand events to.  When unset, every event other than
        READY is dropped.
    ...
    def __init__(
        ...
        def ready_callback() -> None:
            """
            Notifies that the Unleash client is ready.  Only the first call has any
            effect, no matter how many connectors call it or from which thread.
            """
            self.emit_event(
                UnleashReadyEvent(
    ...
    def ready_callback(self) -> Callable[[], None]:
        """
        A zero argument callable that emits the READY event at most once.  Always a
        real callable, even when no callback was configured, so callers never have to
        check for None.
        """
    ...
    def dropped_events(self) -> int:
        """
        Number of events that were never handed to the callback, either because no
        callback was configured or because the queue was full.
        """
        with self._lock:
