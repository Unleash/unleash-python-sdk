...
def test_events_are_dropped_without_a_callback(dispatcher_factory):
...
def test_ready_event_is_not_dropped_without_a_callback(dispatcher_factory):
    dispatcher = dispatcher_factory()
...
def test_ready_callback_is_stable_and_always_callable(dispatcher_factory):
    dispatcher = dispatcher_factory()

    assert callable(dispatcher.ready_callback)
    assert dispatcher.ready_callback is dispatcher.ready_callback


def test_ready_callback_fires_at_most_once(dispatcher_factory):
    received = []
    dispatcher = dispatcher_factory(received.append)
