## Unreleased
* (Minor): Outbound HTTP now happens through one internal `Transport` object instead of the three module-level functions in `UnleashClient.api`. The requests on the wire — urls, methods, headers, bodies, status handling, the retry adapter on feature fetches, and the fatal-URL exceptions that registration re-raises — are exactly what they were. `PollingConnector` now takes a `transport` instead of `url`, `app_name`, `instance_id`, `headers`, `custom_options`, `request_timeout`, `request_retries` and `project`, and `aggregate_and_send_metrics` takes one in first position instead of `url`, `headers`, `custom_options` and `request_timeout`. This only affects code importing from `UnleashClient.api`, `UnleashClient.connectors` or `UnleashClient.periodic_tasks` directly. Nothing changes for code using `UnleashClient`.
...
* (Minor): `UnleashClient.metrics_headers` is now informational. It is still assigned at `initialize_client()` and still reports the headers the metrics request carries, but mutating it no longer changes what is sent — the `Transport` builds that header set per request. Set `unleash_custom_headers` instead.
