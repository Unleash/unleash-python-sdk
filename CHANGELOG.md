...
* (Minor): The in-progress asynchronous client gains an `AsyncPollingConnector`, in `UnleashClient.connectors.async_connector`. It fetches feature state over the asynchronous transport on its own scheduler. Starting it loads the cached state and returns without waiting for the server; the first fetch runs one refresh interval later. With a non-empty cache, READY is therefore emitted when the connector starts rather than after the first fetch. It is not exported from `UnleashClient.connectors` and the asynchronous client does not use it yet. Nothing changes for code using `UnleashClient`.
...
* (Minor): The unused `fl_job` attribute has been removed from `UnleashClient`. It was always `None`, since the connectors own their own jobs, and nothing read it.
